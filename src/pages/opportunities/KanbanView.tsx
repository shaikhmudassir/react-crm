...
const KanbanBoard = (props:KANBANVIEW) => {
  ...
  const columnOrder = ['NEEDS ANALYSIS', 'VALUE PROPOSITION', 'QUALIFICATION', 'CLOSED WON'];
  useEffect(()=>{
    if(data){
      const result = data.reduce((acc:any, opportunity:any) => {
  ...
  return (
    <Container>
    {
      columnOrder && columnOrder.map(stageName=>{
        const stageObj = formattedData?.stages[stageName];
        const opportunities = stageObj?.oppIds.map((oppId:any)=>formattedData.opportunities[oppId])
        // console.log("all-one->",opportunities);
        return <KanbanColumn key={stageObj?.id} column={stageObj} tasks={opportunities} />;
      })
    }
    </Container>
  );
